...
# Returs either an admin server object or a managed user's server object
def getServer(name = ""):
   if name == "":
...
# Returns the section object based on user input
...
# Returns the show object based on user input
def getShow(library):
...
# Returns a stream object given an index
...
# Set the audio and subtitle streams for an episode
def process_episode(episode, audio_idx, sub_idx, server):
   ...
   print("%s:" % episode)
   print("\tAudio Track: %s - %s" % (audio_stream.languageCode, audio_stream.title))
   if (subtitle_stream is not None):
      print("\tSubtitle Track: %s - %s" % (subtitle_stream.languageCode, subtitle_stream.title))
   else:
   ...
   # Audio Query String
   aq = '/library/parts/%s?audioStreamID=%s&allParts=1' % (part.id, audio_track_plex_id)

   # Subtitle Query String
   sq = '/library/parts/%s?subtitleStreamID=%s&allParts=1' % (part.id, sub_track_plex_id)
   
   server.query(aq, method = server._session.put)
   server.query(sq, method = server._session.put)

   print()

# Process a season's worth of episodes
